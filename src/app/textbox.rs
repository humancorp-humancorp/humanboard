...
impl Humanboard {
    ...
    pub fn start_textbox_editing(
        ...
        let current_text = if let Some(ref board) = self.canvas.board {
        ...
        if let Some(text) = current_text {
                ...
                |this, input, event: &gpui_component::input::InputEvent, cx| {
                    match event {
                        gpui_component::input::InputEvent::Change { .. } => {
                            ...
                            if let Some(item_id) = this.textbox.editing_id {
            ...
            self.textbox.editing_id = Some(item_id);
            self.textbox.input = Some(input);
            cx.notify();
        }
    }
    ...
    pub fn finish_textbox_editing(&mut self, cx: &mut Context<Self>) {
        if let Some(item_id) = self.textbox.editing_id.take() {
            if let Some(input) = self.textbox.input.take() {
                let new_text = input.read(cx).text().to_string();
    ...
    pub fn update_fps(&mut self) {
        let now = Instant::now();
        let frame_time = now.duration_since(self.system.last_frame);
        self.system.last_frame = now;
        self.system.frame_count += 1;

        self.system.frame_times.push(frame_time);
        if self.system.frame_times.len() > 60 {
            self.system.frame_times.remove(0);
        }
    }

    pub fn calculate_fps(&self) -> f32 {
        if self.system.frame_times.is_empty() {
            return 0.0;
        }
        let avg_frame_time: Duration =
            self.system.frame_times.iter().sum::<Duration>() / self.system.frame_times.len() as u32;
